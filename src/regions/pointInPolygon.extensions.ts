...
declare module "leaflet" {
    interface Polyline {
        ...
        getWindingNumber(p: LatLng): number;
    }

    interface LatLng {
        isLeft(p1: LatLng, p2: LatLng): number;
    }
}
 ...
 */
Polyline.prototype.contains = function (p: L.LatLng): boolean {
    //"use strict";
 ...
 * @see {@link https://github.com/Fragger/Leaflet.Geodesic Leaflet.Geodesc} for information about Leaflet.Geodesc by Fragger.
 */
Polyline.prototype.getWindingNumber = function (p) { // Note that L.Polygon extends L.Polyline
    var i,
        isLeftTest,
        n,
        vertices,
        wn; // the winding number counter
    function flatten(a: LatLng[] | LatLng[][] | LatLng[][][]): LatLng[] {
        var flat;
        flat = ((Array.isArray ? Array.isArray(a) : Util.isArray(a)) ? a.reduce(function (accumulator: LatLng[], v, i, array) {
            return accumulator.concat(Array.isArray(v) ? flatten(v) : v);
        }, [])
            : a);
        return flat as LatLng[];
    }
